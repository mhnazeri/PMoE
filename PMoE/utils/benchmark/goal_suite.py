...
class PointGoalSuite(BaseSuite):
    ...
    def ready(self):
        ...
        self._local_planner.set_route(
            self._start_pose.location, self._target_pose.location
        )
    ...
    def apply_control(self, control):
        ...
        # reward for steer
        if self.command == 5:  # go  straight
            if abs(control.steer) > 0.2:
                self.reward -= 20
            self.reward += min(35, speed * 3.6)
        elif self.command == 2:  # follow  lane
            self.reward += min(25, speed * 3.6)
        elif self.command == 3:  # turn  left ,steer should be negative
            if control.steer > 0:
                self.reward -= 15
            if speed * 3.6 <= 20:
        ...
        elif self.command == 4:  # turn  right
            if control.steer < 0:
                self.reward -= 15
            if speed * 3.6 <= 20:
                self.reward += speed * 3.6
        ...
        # reward  for  lane invasion  and  collision
        if self.invaded:
            self.reward -= 100
        if self.collided:
            self.reward -= 100
        elif self.col_is_failure:
            self.reward -= 100

        # terminal  state
        done = False
        if self.is_success() or self.is_failure():
            done = True
    ...
    def _setup_sensors(self):
        ...
        if self.viz_camera:
            ...
            viz_camera_bp.set_attribute("image_size_x", "800")
            viz_camera_bp.set_attribute("image_size_y", "600")
