"""source: https://github.com/dotchen/LearningByCheating/blob/release-0.9.6/bird_view/utils/carla_utils.py"""
import collections
import queue
import weakref
import time
...
def process(observations):
    ...
    result["control"] = np.array(
        [control[0], pedal], dtype=np.float32
    )  # np.float32(control)
    ...
    measurements = [
        x if isinstance(x, np.ndarray) else np.float32([x]) for x in measurements
    ]
